...
bool linkedqueue::search(int item)
{
    if (head == NULL)
        return false;
    else
    ...
    // The control should not reach here without returning a value,
    // but to satisfy the compiler, we can return false as a default.
    return false;
}
